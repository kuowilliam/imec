...
class PedestrianDetectionDecoder(nn.Module):
    """
    decode multi-scale camera features into CenterNet-style predictions.
    - using top-down FPN pathway from fused s16 to s8.
    - using center-net style prediction head.
    """
    def __init__(
        self,
        fused_channels=256,
        camera_s8_channels=192,
        camera_s4_channels=96,
        ...
        self.s8_lateral = nn.Conv2d( # also convert camera s8 to decoder 128 channels
            camera_s8_channels,
            decoder_channels,
            kernel_size=1,
        )
    ...
    def forward(self, fused_s16, camera_s8, camera_s4):
        ...
        decoded_s8 = F.interpolate(
            decoded_s16,
            size=camera_s8.shape[-2:],
            mode="bilinear",
            align_corners=False,
        )
        decoded_s8 = self.s8_refine( # fused with camera s8 and refine
            decoded_s8 + self.s8_lateral(camera_s8)
        )

        decoded_s4 = F.interpolate(
            decoded_s8,
            size=camera_s4.shape[-2:],
            mode="bilinear",
            align_corners=False,
        )
        decoded_s4 = self.s4_refine(
            decoded_s4 + self.s4_lateral(camera_s4)
        )
